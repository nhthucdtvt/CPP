...
void XuLyDuLieu(int *a,int *b,int *c,int n,int m)
{
	...
	// Ý tưởng thuật toán : Thiết lập vòng lặp cho chạy từ 0 -> <n hay <m tùy ý (do lúc này n = m) rồi ta tiến hành tìm phần tử nhỏ nhất của từng vị trí trong mảng tương ứng và lưu chúng vào trong mảng c .
	if(n==m)
	{
	...
	// Ý tưởng thuật toán : Cần 2 vòng lặp . Vòng lặp đầu tiên ta cho chạy từ 0 -> <m (do mảng b có chiều dài bé hơn mảng a) rồi tiến hành tìm phần tử nhỏ nhất của từng vị trí trong mảng tương ứng và lưu chúng vào trong mảng c . Tiếp theo vòng lặp thứ 2 ta cho chạy từ m -> <n (lúc này các phần tử dư ra chỉ là của mảng a),ta đem các phần tử đó ghép vào mảng c . 
	else if(n>m)
	{
		for(int i=0;i<m;i++)
		{
	...
	// Ý tưởng thuật toán : Cần 2 vòng lặp . Vòng lặp đầu tiên ta cho chạy từ 0 -> <n (do mảng a có chiều dài bé hơn mảng b) rồi tiến hành tìm phần tử nhỏ nhất của từng vị trí trong mảng tương ứng và lưu chúng vào trong mảng c . Tiếp theo vòng lặp thứ 2 ta cho chạy từ n -> <m (lúc này các phần tử dư ra chỉ là của mảng b),ta đem các phần tử đó ghép vào mảng c . 
	else if(n<m)
	{
		for(int i=0;i<n;i++)
		{
