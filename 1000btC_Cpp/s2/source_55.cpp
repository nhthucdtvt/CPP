...
void xulydulieu(float &x,int &n)
{
	...
	for(int i=0;i<=n;i++)
	{
		s+=pow(-1,i+1)*(pow(x,2*i)/giaithua(2*i)); // ở đây s đang ở kiểu số thực và pow cũng đang ở kiểu số thực nên kết quả luôn đúng,nếu s ở số thực mà vế sau nó ở dạng số nguyên thì ta phải nhân thêm 1.0 vào mới đúng được.
	}
	printf("\nKet qua la:%f",s);
}
void main()
