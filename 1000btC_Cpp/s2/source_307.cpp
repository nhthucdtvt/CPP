...
void xulydulieu(float a[MAX][MAX],int n,int m)
{
	...
	for(int i=0;i<m;i++)
	{
		float tong=0;		// tong=0 phải để trong vòng lặp thứ nhất để mỗi lần sau khi cộng xong 1 dòng thì tổng lại trả về 0 và tiếp tục cộng dòng mới,nếu không thì dòng mới sẽ có tổng=tổng của nó + tổng dòng trước đó.
		for(int j=0;j<n;j++)
		{
			if(a[i][j]>=0)
			{
				tong+=a[i][j];
				b[i]=tong;	// lưu giá trị tổng dòng vào mảng b
			}
		}		
	}
	for(int i=0;i<m;i++)
		printf("\nTong dong %d la %f",i,b[i]);
}
void main()
{
	float a[MAX][MAX];
