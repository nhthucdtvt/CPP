...
void tinhtrungbinhcongcacsoduong(float a[MAX][MAX],int n,int m)
{
	int dem=0;
	...
	if(dem!=0)
	{
		...
		printf("\nTrung binh cong cac so duong la:%f",trungbinhcong);
	}
	else
		printf("\nTrong ma tran khong ton tai so duong");
}
void main()
{
	float a[MAX][MAX];
	...
	printf("\n>>>>>>>>>>>>>>>MA TRAN VUA NHAP LA:<<<<<<<<<<<<<<<<<<<<<\n");
	xuatmatran(a,n,m);
	tinhtrungbinhcongcacsoduong(a,n,m);
