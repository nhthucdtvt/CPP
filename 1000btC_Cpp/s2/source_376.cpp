...
void xulydulieu(int a[MAX][MAX],int n,int m)
{
	int b[MAX],i,j;
	int dem=0;
	printf("\nCac gia tri chan co trong ma tran la:");
	for(i=0;i<m;i++)
	{
		for(j=0;j<n;j++)
		{
			if(a[i][j]%2==0)
			{
				printf("%4d",a[i][j]);
				b[dem]=a[i][j];
				dem++;	// dem++ phải để ở sau dòng b[dem]=a[i][j] thì mới ra kết quả đúng được.
			}
		}
	}
	printf("\nSo luong cac gia tri chan co trong ma tran la:%d",dem);
	printf("\nCac gia tri chan sau khi sap xep giam dan la:");
	for(i=0;i<dem;i++)
	{
		for(j=i+1;j<dem;j++)
		{
