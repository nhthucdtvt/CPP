...
void timlonnhattrenduongcheochinh(float a[MAX][MAX],int m,int n)
{
	float max;
	for(int i=0;i<m;i++)
	{
		max=a[0][0];	// gán max=phần tử đầu tiên là a[0][0].
		if(a[i][i]>max)
			max=a[i][i];
	}
	printf("\nGia tri lon nhat tren duong cheo chinh la:%f",max);
}
void main()
{
	float a[MAX][MAX];
	...
	timlonnhattrenduongcheochinh(a,m,n);
