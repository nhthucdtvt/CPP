...
void timnhonhattrencot(float a[MAX][MAX],int n,int m,int &x)
{
	float min;
	do{
		printf("\nNhap vao so cot can tim gia tri nho nhat:x=");
		scanf("%d",&x);
		if(x<0||x>n-1)
	...
	for(int i=0;i<m;i++)
	{
		...
		printf("\nCot:%d co gia tri nho nhat la:%f",x,min);
}
void main()
{
	float a[MAX][MAX];
