...
void timlonnhattrenduongcheophu(float a[MAX][MAX],int m,int n)
{
	float max;
	for(int i=0;i<m;i++)
	{
		max=a[0][m-1];	// gán max=phần tử đầu tiên là a[0][m-1].
		if(a[i][i]>max)
			max=a[i][m-i-1];
	}
	printf("\nGia tri lon nhat tren duong cheo phu la:%f",max);
}
void main()
{
	float a[MAX][MAX];
	...
	timlonnhattrenduongcheophu(a,m,n);
