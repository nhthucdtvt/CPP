...
void ThanksTeacherVoQuangDieuHa()
{
	int i,j;
	for(i=1;i<=100;i++)
	{
		...
		printf("\n\n\t\t\tSend ......... To Teacher:Vo Quang Dieu Ha  ");
		printf("\n\n\t\t\t\tLoading...%d %c", i, '%');
		printf("\n\n\t\t\t------------------------------------------");
		...
		if(i==55)
		{
			...
			printf("\n\n\tThoi gian troi qua that nhanh chong,moi do ma da 10 tuan troi qua ! ");
			Sleep(4000);
		}
		if(i==66)
		{
			textcolor(13); // Tô chữ màu hồng !
			printf("\n\nChung em rat cam on Co da tan tinh chi day chung em trong suot 10 tuan qua ! ");
			Sleep(4000);
		}
		if(i==77)
		{
...
{
	FILE * pFile;
	pFile = fopen("D:\\InPut.txt", "r+t"); // Tìm kiếm và mở File InPut.txt ở trong ổ đĩa D để đọc dữ liệu trong File vào chương trình .
	if(pFile==NULL) // Nếu File rỗng (không tìm thấy File) => báo lỗi và dừng chương trình .
	{
		textcolor(13); // Tô màu hồng . 
		printf("\nKhong Doc Duoc File InPut.txt Trong O D \n"); // Báo lỗi không đọc được file InPut.txt trong ổ đĩa D .
		printf("\nMoi Ban Tao Ra D:\\InPut.txt De Nhap Du Lieu Cho Chuong Trinh "); // Nhắc nhở người dùng tạo ra File InPut.txt để nhập dữ liệu cho chương trình .
	}
	else // Nếu đã tìm được File thì đọc dữ liệu trong File vào chương trình .
	{
		textcolor(13); // Tô màu hồng . 
		printf("\nDu Lieu Tu D:\\InPut.txt Da Duoc Doc Vao Chuong Trinh !\n"); // Thông báo đã đọc thành công dữ liệu từ File InPut.txt vào chương trình .
		textcolor(7); // Trả về màu chữ bình thường .
...
{
	FILE * pFile;
	pFile = fopen ("D:\\OutPut.txt","wt"); // Mở tập tin OutPut.txt trong ổ đĩa D để ghi dữ liệu.Tập tin sẽ được tạo mới nếu chưa có,ngược lại nếu tập tin có sẵn thì dữ liệu trước đó sẽ bị xóa hết .
	fprintf(pFile,"%d\n",n); // Ghi số lượng các phân số vào đầu File OutPut.txt trong ổ đĩa D .
	for(int i=0;i<n;i++)
	{
		fprintf(pFile,"%d %d\n",a[i].tuso,a[i].mauso); // Ghi giá trị của các phân số sau khi rút gọn và được sắp xếp tăng dần vào File OutPut.txt trong ổ D .
	}
	textcolor(14); // Tô chữ màu vàng .
	printf("\nCac Phan So Sau Khi Sap Xep Tang Dan Duoc Ghi Vao Trong D:\\OutPut.txt ! ");
	printf("\n");
	printf("\nMoi Ban Vao D:\\OutPut.txt De Xem Ket Qua ! ");
	printf("\n");
	fclose (pFile); // Đóng file lại .
}
void MeNu()
{
	...
	printf("\n---------------------------------- MeNu ----------------------------------------");
	printf("\n1. Doc Du Lieu Cac Phan So Tu D:\\InPut.txt Vao Chuong Trinh ");
	...
	printf("\n5. Ghi Noi Dung Cac Phan So Sau Khi Da Sap Xep Vao D:\\OutPut.txt \n");
	textcolor(13);
	printf("\n\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3");
	textcolor(10);
	printf("\n6. \3\3\3 Loi Chuc Tot Dep Nhat Chung Em Gui Den Teacher : Vo Quang Dieu Ha \3\3\3  \n");
	textcolor(13);
	printf("\n\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3\3");
	textcolor(10);
	...
	printf("\n");
	quaylai:textcolor(15);
	printf("\nNhap Vao Lua Chon Cua Ban:");
	scanf("%d",&luachon);
	textcolor(7);
	printf("\n");
	printf("\nKet Qua:");
	if(luachon==1) // Lựa chọn đọc dữ liệu các phân số từ File InPut.txt trong ổ đĩa D vào chương trình .
	{
	...
	else if(luachon==5) // Lựa chọn ghi kết quả các phân số sau khi sắp xếp tăng dần vào File: OutPut.txt trong ổ D .
	{
		system("cls"); // Xóa đi hết mọi dữ liệu đã thực thi trước đó .
		RutGonPhanSo(a,n); // Rút gọn các phân số .
		HeapSort(a,n); // Sắp xếp các phân số vừa được rút gọn tăng dần bằng thuật toán HeapSort .
		GhiFileOutPut(a,n); // Ghi các kết quả vừa sắp xếp xong vào D:\\OutPut.txt .
		goto quaylai1; // Dùng hợp ngữ - chức năng để quay trở lại bảng MeNu cho lần lựa chọn tiếp theo .
	}
	else if(luachon==6) // Lựa chọn in ra lời chúc tốt đẹp nhất của tụi em dành cho Cô .
	{
		ThanksTeacherVoQuangDieuHa(); // In ra lời chúc tốt đẹp nhất mà chúng em gửi tặng đến Cô .
		system("cls"); // Xóa đi hết mọi dữ liệu đã thực thi trước đó .
		goto quaylai1; // Dùng hợp ngữ - chức năng để quay trở lại bảng MeNu cho lần lựa chọn tiếp theo .
	}
